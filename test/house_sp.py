...
def delete_db():
  ...
  cur=conn.cursor()
  cur.execute('delete from house_all')
  cur.close()
  conn.commit()
  conn.close()
...
def insert_db(j):
  try:
    conn=pymysql.connect(**config)
    cur=conn.cursor()
    sql="insert into house_all (province,city,target,currentPrice,bidCount,marketPrice,applyCount,viewerNum,startTime,stopTime,status,department,addPrice,deposit,startPrice,period,property,pass_in) values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
    data=(j['province'],j['city'],j['target'],j['currentPrice'],j['bidCount'],j['marketPrice'],j['applyCount'],j['viewerNum'],j['startTime'],j['stopTime'],j['status'],j['department'],j['addPrice'],j['deposit'],j['startPrice'],j['period'],j['property'],j['pass_in'])
    #print(data)
